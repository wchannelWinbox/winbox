...
gtf 1360 768 60 | grep "Modeline .*" | sed -e 's/  Modeline //g' | xargs xrandr --newmode
xrandr | grep ".* connected" | sed -e 's/ connected.*//g' | tr -d '\n' | xargs -0 -I name xrandr --addmode name 1360x768_60.00
xrandr | grep ".* connected" | sed -e 's/ connected.*//g' | tr -d '\n' | xargs -0 -I name xrandr --output name --mode 1360x768_60.00
