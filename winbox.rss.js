...
(function() {
	...
	try{
			if( jQuery.isEmptyObject(CONFIG.channel) || CONFIG.channel.rss == "" || !CONFIG.channel.rss[0].enabled ) { $("#rss").hide(); $("#rss [id^=rss]").remove(); }else{
				$("#rss").show();
			}
			if( (parseInt(Date.now()/1000, 10) % (60*CONFIG.times.rss_interval))==0 || (NEWS.index==-1 && RSS.index==-1) ) {
				/*console.clear();*/ $.getJSON("data/news.json?0", function(news) {
					if( news.length>0 && CONFIG.channel.rss != "" && CONFIG.channel.rss[0].enabled) {
